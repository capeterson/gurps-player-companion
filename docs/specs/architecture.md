...
This is what lets the same Zod schema validate a request on the server, an
optimistic write in the client, and a replayed op in the SW.
...
- **`src/sw/`** — service worker registration + offline replay.
...
Schema is Drizzle (`src/server/db/schema.ts`); migrations are **hand-written
SQL** under `src/server/db/migrations/` (trigger logic is not auto-generated by
drizzle-kit).
...
- `vitest run` — client component/hook tests (jsdom via happy-dom;
  `fake-indexeddb` for Dexie).
- `playwright test` — end-to-end.
- `npm run check` = `lint` (Biome) + `typecheck` (`tsc --build`) + tests +
  `openapi:check` (contract drift). This is the gate to run before finishing a
  change.
