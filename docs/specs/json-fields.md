...
| `inventory_items.armor` | `armorData` (inventory.ts), nullable; nests `typedDr` (`typedArmorDr` — per-damage-type DR overrides for cut/imp/pi/pi−/pi+/pi++/burn/corr/fat/tox, defaults to `{}`; `resolveDr` in armorDr.ts falls back to `drCrushing` for `cr` then base `dr`) plus an optional `db` (armor Defense Bonus from Deflect enchantments, stacks with shield DB via `sumArmorDb`) | REST inventory create/update (`inventoryItemCreate` / `inventoryItemUpdate`); sync per-field validator |
