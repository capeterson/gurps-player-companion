...
export function buildCharacterDetail(input: SummaryInput): CharacterDetail {
  const { character, traits, skills, inventory, campaign } = input;
...
export function buildInventoryItemOut(
  ...
  return {
    ...
    armor: item.armor as Record<string, unknown> | null,
    weaponData: item.weaponData as Record<string, unknown> | null,
