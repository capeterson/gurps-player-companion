...
describe('Armor map integration', () => {
  ...
  it.each(['2', '3', '5', '10', '100', 'ignore'])(
    ...
    (divisor) => {
      ...
      const context = dialog.getByLabelText('Incoming attack context');
      expect(context).toHaveTextContent('Vitals');
      expect(context).toHaveTextContent('Impaling');
      expect(context).toHaveTextContent(divisor === 'ignore' ? 'Ignore' : `(${divisor})`);
      expect(dialog.queryByLabelText('Hit location')).not.toBeInTheDocument();
  ...
  it('retains invalid input and blocks submission rather than treating a bad divisor as normal DR', () => {
    ...
    change('Armor penetration', '__custom');
    change('Custom armor divisor', '-2');
    expect(screen.getByRole('alert')).toHaveTextContent('positive armor divisor');
    expect(screen.getByRole('button', { name: /Incoming damage/ })).toBeDisabled();
    expect(bumpHp).not.toHaveBeenCalled();
    change('Custom armor divisor', '(2)');
    expect(screen.getByRole('button', { name: /Incoming damage/ })).toBeEnabled();
    const nextDialog = openDamage();
    fireEvent.change(nextDialog.getByLabelText('Basic damage'), { target: { value: '10' } });
    expect(nextDialog.getByRole('button', { name: /Apply/ })).toBeEnabled();
  });
});
