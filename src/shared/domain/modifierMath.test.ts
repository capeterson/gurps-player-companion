...
describe('computeTraitCost', () => {
  ...
  it('+50% enhancement on a 20-pt advantage = 30', () => {
    expect(computeTraitCost(20, [enh('Area Effect', 50)]).total).toBe(30);
  });

  it('-40% limitation on a 20-pt advantage = 12', () => {
    expect(computeTraitCost(20, [lim('Mitigator', -40)]).total).toBe(12);
  });
  ...
  it('adds flat modifiers after percent scaling', () => {
    const r = computeTraitCost(20, [enh('a', 50), flat('b', 5)]);
